...
class OSCError(RuntimeError):
    """Base exception for OSC communication and capture failures."""
...
class Insta360OSCClient:
    """Minimal synchronous OSC client for on-device stitched photos."""
    ...
    def command_status(self, command_id: str) -> dict[str, Any]:
        return self._json_request(
            "POST", "/osc/commands/status", payload={"id": command_id}
        )

    def get_options(self, *option_names: str) -> dict[str, Any]:
        response = self.execute(
            "camera.getOptions", {"optionNames": list(option_names)}
        )
