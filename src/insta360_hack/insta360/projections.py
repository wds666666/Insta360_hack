...
class FFmpegTimeoutError(OSCError):
    """FFmpeg did not finish a projection within its configured deadline."""
...
def _check_square_jpeg(path: Path, expected_size: int) -> dict[str, Any]:
    try:
        ...
        raise OSCError(f"投影结果无法解码: {path.name}: {exc}") from exc
    if image_format.upper() != "JPEG":
        raise OSCError(f"投影结果不是 JPEG: {path.name}")
    if (width, height) != (expected_size, expected_size):
        raise OSCError(
            f"投影结果尺寸错误: {path.name}: {width}x{height}，"
    ...
    *,
    ...
    try:
        ...
        raise FFmpegTimeoutError(
            f"FFmpeg 生成 {destination.name} 超时（{timeout:g} 秒）"
        ) from exc
...
def export_projection_bundle(
    ...
    *,
    ...
    if ffmpeg_timeout <= 0:
        raise OSCError("ffmpeg_timeout 必须大于 0")

    resolved_ffmpeg = shutil.which(ffmpeg_bin)
    if resolved_ffmpeg is None:
        raise OSCError("未找到 FFmpeg，请确认已安装且包含 v360 filter")
