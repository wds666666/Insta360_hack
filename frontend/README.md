...
- 全景图和空间说明。可以点选、拖入，或按 `Ctrl+V` / `⌘V` 粘贴图片。按下「生成触觉地图」后创建一次 `img-to-3d`。有任务还在进行时不能再开新的。
- 生成方式默认全自动，提交后一直做到模型。选「先看优化图」时，优化图完成后停住，要点「用这张图生成触觉模型」才继续。
- 进度列出后端返回的九个步骤。优化参考图和网格生成进行中显示「已进行」，结束后保留「用时」。
- 全景原图：拍摄到的视觉空间。一有地址就显示，本地选中的图会先占这个位置。
- 空间结构：去掉人物、杂物和文字后的优化图。优化进行时，这里也显示已经用了多久。
...
| 创建 | `POST /api/v1/runs`，表单字段 `workflow_id=img-to-3d`、`prompt`、`image`、`mode`（`auto` 或 `confirm`） |
